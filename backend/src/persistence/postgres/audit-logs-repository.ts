...
export class PostgresAuditLogsRepository {
  async list(client: PoolClient): Promise<AuditLogRecord[]> {
    const result = await client.query(`
      select id, at, actor_type, actor_id, action, entity_type, entity_id, metadata
      from audit_logs
      order by at desc
    `);
  ...
  async appendOne(client: PoolClient, item: AuditLogRecord): Promise<void> {
    await client.query(
      `insert into audit_logs(id, at, actor_type, actor_id, action, entity_type, entity_id, metadata)
      ...
      [item.id, item.at, item.actorType, item.actorId, item.action, item.entityType, item.entityId, toJson(item.metadata ?? {})],
    );
  }
