  ...
  nodes.orders.innerHTML = orders.map((order) => `
    <article class="order-card">
      ...
      <div class="order-grid">
        <div class="card" style="padding: 14px;"><div class="eyebrow">Courier</div><div style="margin-top: 8px;">${escapeHtml(order.tracking.assignedDriverName || 'Waiting for courier assignment')}</div><div class="muted">Pickup ETA: ${formatMinutes(order.tracking.driverEtaToPickupMinutes)}</div><div class="muted">${order.isBatchedTrip ? `Batched trip - ${order.tripOrderCount} orders in this run` : 'Single-order trip'}</div></div>
...
function renderReports(profile, report, orders) {
  ...
  nodes.reportCourierSummary.innerHTML = (report.byTrip || []).length ? (report.byTrip || []).map((row) => `<div class="list-row"><span>${escapeHtml(row.driverName)} - ${row.orderCount} orders</span><strong>${row.completedAt ? 'Completed trip' : 'Open trip'}</strong></div>`).join('') : ((report.byCourier || []).map((row) => `<div class="list-row"><span>${escapeHtml(row.driverName)}</span><strong>${row.totalOrders} orders</strong></div>`).join('') || '<div class="muted">No courier data yet.</div>');
